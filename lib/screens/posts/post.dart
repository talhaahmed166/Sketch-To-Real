// ignore_for_file: no_leading_underscores_for_local_identifiers, use_key_in_widget_constructors, unnecessary_null_comparison
...
class Post extends StatefulWidget {
  ...
  factory Post.fromDocument(doc) {
    return Post(
      ...
      isApproved: doc.data()["isApproved"], //getApproved(),
...
//Note:to delete Post,ownerId  % and currentUserId must be equal, so they can be used interchangeably
  deletePost() async {
    await postRef.doc(ownerId).collection('userPosts').doc(postId).delete();
    // await timelineRef.doc().delete();
    //     .get()
    //     .then((doc) {
    //   if (doc.exists) {}
    ...
    // my code to delete data from timeline

    timelineRef.doc(postId).delete();
    ...
    BotToast.showText(text: "Post Deleted  Refresh Page");
  }
  ...
  initState() {
    ...
    postRef
      ...
      setState(() {
        Map likeArr = ds.get("likes");
        likeCount = getLikeCount(likeArr);
        print('${likeCount} hani is here');
      });
    });
  }

  int getLikeCount(likeArr) {
    if (likeArr == null) {
      return 0;
    }
    int count = 0;
    likeArr.values.forEach((val) {
      if (val) {
        count += 1;
      }
    });
